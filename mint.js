const bitcoin = require("bitcoinjs-lib");
const axios = require("axios");
const ecc = require("tiny-secp256k1");
const { ECPairFactory } = require("ecpair");

// Initialize ECPair with the required elliptic curve implementation
const ECPair = ECPairFactory(ecc);

const network = bitcoin.networks.testnet;


const yourAddress = "tb1qkz9d6g4m39626wlrn9m6unv5jvtrk2ktxqd2lu";
const privateKey = "cNR2qRtwVf6hrRVi9Kfomj59KSJQhjChqLG58au8c22aJt4G9uwP";

const runeId = "4257852:123"; 
const mintAmount = 1000;

function encodeMintRunestone(runeId, amount) {
	return Buffer.from(`RUNE:MINT:${runeId}:${amount}`).toString("hex");
}

async function mintRune() {
	try {
		// Fetch UTXOs
		const utxoResponse = await axios.get(
			`https://mempool.space/testnet/api/address/${yourAddress}/utxo`
		);
		const utxos = utxoResponse.data;
		const utxo = utxos.find((u) => u.value >= 10000); // 0.001 tBTC
		if (!utxo) throw new Error("Insufficient tBTC");

		// We need to get the scriptPubKey for the input
		const addressInfo = await axios.get(
			`https://mempool.space/testnet/api/address/${yourAddress}`
		);
		const scriptPubKey =
			addressInfo.data.scriptPubKey ||
			bitcoin.address
				.toOutputScript(yourAddress, network)
				.toString("hex");

		// Create PSBT
		const psbt = new bitcoin.Psbt({ network });
		psbt.addInput({
			hash: utxo.txid,
			index: utxo.vout,
			witnessUtxo: {
				script: Buffer.from(scriptPubKey, "hex"),
				value: utxo.value,
			},
		});

		// Add OP_RETURN for mint
		const runestoneData = encodeMintRunestone(runeId, mintAmount);
		const data = Buffer.from(runestoneData, "hex");

		psbt.addOutput({
			script: bitcoin.script.compile([bitcoin.opcodes.OP_RETURN, data]),
			value: 0,
		});

		psbt.addOutput({
			address: yourAddress,
			value: 546, // Dust limit
		});

		const fee = 1000; // Adjust based on network conditions
		psbt.addOutput({
			address: yourAddress,
			value: utxo.value - 546 - fee,
		});

		// Sign
		const keyPair = ECPair.fromWIF(privateKey, network);
		psbt.signInput(0, keyPair);
		psbt.finalizeAllInputs();

		// Broadcast
		const txHex = psbt.extractTransaction().toHex();
		const broadcastResponse = await axios.post(
			"https://mempool.space/testnet/api/tx",
			txHex
		);
		console.log("Minted! TxID:", broadcastResponse.data);
	} catch (error) {
		console.error("Error:", error.message);
		if (error.response) {
			console.error("Response data:", error.response.data);
		}
	}
}

mintRune();
